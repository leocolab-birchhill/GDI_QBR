    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Simulate an email to{" "}
          <code className="rounded bg-muted px-1">qbr@gdi.com</code>. Drives the
          full classify → extract → DB → reply pipeline (works offline without
          an OpenAI key).
        </p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            />
            <Field
              label="To (the BR bot mailbox)"
              value={form.toEmail}
              onChange={(v) => setForm({ ...form, toEmail: v })}
            />
            {mailbox && (
              <p className="rounded-md bg-accent px-3 py-2 text-xs text-accent-foreground">
                ↩ The bot will reply <strong>from {mailbox}</strong>{" "}
                <strong>to {form.fromEmail || "the From address"}</strong>.
              </p>
            )}
            <Field
              label="Subject"
          ...
          <CardContent>
            {result ? (
              ...
              <p className="text-sm text-muted-foreground">
                Send an email to see the parsed result and auto-reply.
              </p>
            )}
          </CardContent>
        </Card>
