...
async function callAndValidate<S extends z.ZodTypeAny>(
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    try {
      ...
      console.warn(
        "[ai] schema validation failed (attempt %d):",
        attempt + 1,
        parsed.error.issues,
      );
    } catch (err) {
      console.warn(
        "[ai] JSON parse failed (attempt %d):",
        attempt + 1,
...
export async function detectEmailLanguageAI(input: {
  ...
  const result = await callAndValidate(LanguageSchema, system, user, {
...
export async function classifyEmailIntent(input: {
  ...
  const system = `You classify inbound Business Review (BR) emails from internal operations staff. Respond ONLY with JSON: {"intent": one of [${INTENT_VALUES}], "confidence": 0..1, "reasoning": string}. Pick the single best intent.

Intent guide:
- CREATE_QBR: start/create a new BR OR onboard a brand-new client/account (often "Start BR - Client - Q# YYYY", but ALSO "create a new account", "create new account", "set up a new client", "onboard <Client>", "add a new client/account", "nouveau compte", "créer un compte/client"). Treat any request to create an account, client, or BR as CREATE_QBR — there is no separate account-creation intent.
...
export async function extractQbrDataFromEmail(input: {
  ...
  const system = `You extract structured BR data from operator emails. Respond ONLY with JSON matching this shape:
...
Dates: "meetingDate" is THIS BR's meeting date; "nextMeetingDate" is the proposed date of the NEXT BR/meeting (e.g. "next review July 16", "let's meet again on..."). Use ISO yyyy-mm-dd when a concrete date is given; otherwise leave null. Never guess a date that was not stated.`;
...
export async function rewriteClientSafe(input: {
  ...
  const system = `You rewrite rough internal operator notes into professional, client-facing language for a Business Review.
...
export async function generateMissingInfoQuestions(input: {
  ...
  const system = `You generate a short checklist of missing-info questions for a BR. Respond ONLY with JSON: {"questions":[{"field":string,"question":string}]}. Ask only for what is missing. Be concise.`;
...
export async function generateSlideContent(input: {
  ...
  const system = `You produce slide-ready content for a 7-slide BR deck that must match our approved house template in structure and tone. Respond ONLY with JSON matching the provided structure. Keep priority explanations to 1-2 sentences and What's Next details to one concise sentence. Never invent metric values; keep "${TO_CONFIRM}" where unconfirmed. Match the exact structure, slide order, section names, table columns, item counts, and level of detail of the reference template below. Use the reference ONLY for format — never copy its specific facts, numbers, names, or dates.

${reference}`;
  const user = `Structured BR data:\n${JSON.stringify(input.data, null, 2)}`;
  return callAndValidate(SlideContentSchema, system, user);
}

export async function reviewForClientSafety(input: {
  text: string;
}): Promise<ReviewResult> {
  const system = `You review text for client-safety in a BR deck. Flag blame, internal politics, sensitive comments, or invented facts. Respond ONLY with JSON: {"isClientSafe": boolean, "issues": string[], "suggestedRewrite": string|null}.`;
...
export async function summarizeQbrForVp(input: {
  ...
  const system = `You write a one-month BR preparation summary for a VP. Respond ONLY with JSON: {"summary": string, "missingFields": string[], "itemsNeedingVpReview": string[]}. The summary should cover: current status, open follow-ups, draft priority items, available metrics, missing metrics, what's next, items needing VP review.`;
...
/**
 * Agent answer mode: answer a free-form BR question conversationally and
 * specifically, grounded in the BR context loaded from the DB. Falls back to a
...
export async function answerQbrQuestion(input: {
  ...
  const system = `You are the GDI BR assistant answering an internal operator's question about a specific Business Review. Answer conversationally, specifically, and concisely using ONLY the provided BR context — never invent metric values, owners, or dates.
The context includes: meetingDate (this quarter's meeting), nextMeetingDate (the next BR), previousQbrNotes, and latestDeck (a summary of the most recent generated deck — the "last presentation"). USE these to answer date questions ("when is the next meeting?") and recall questions ("what did we present last time?"). If a date or fact is null/absent, say it isn't on file yet rather than guessing.
 ...
 * GDI BR assistant, grounded in the full BR context. The agent ALWAYS answers
...
export async function runQbrAgent(input: {
  ...
  const actionsList =
    "generate_draft, send_deck, approve, revise, finalize, send_survey, none";
  const languageRule =
    input.locale === "en"
      ? `LANGUAGE (STRICT): Reply in ENGLISH ONLY. Write "reply" and every "nextActions" item entirely in English, even though the BR context/data below may be in French. Do not switch to French.`
      : `LANGUE (STRICT) : Répondez UNIQUEMENT en français (fr-CA), dans un ton professionnel. Rédigez « reply » et chaque élément de « nextActions » entièrement en français, même si le contexte/les données du BR ci-dessous sont en anglais. Ne passez pas à l'anglais.`;
  const system = `You are the GDI BR assistant. You converse with an internal operator over email and have the FULL context of one BR (account, quarter, meetingDate, nextMeetingDate, previousQbrNotes, commitments, priorities, metrics, upcoming items, missing info, approvals, deck versions, latestDeck content summary, recent emails). Behave like a helpful chatbot that also takes real actions and remembers prior meetings.
...
- Use meetingDate / nextMeetingDate to answer "when is the meeting / next BR?" and latestDeck + previousQbrNotes to answer "what did we present / discuss last time?". If a value is null/absent, say it's not on file yet — do not guess.
...
Respond ONLY with JSON: {"reply": string, "action": one of [${actionsList}], "revisionNote": string|null, "nextActions": string[]}.`;
  ...
  const user = `User email:\n${input.message}\n\nJust captured (already stored this turn):\n${
    input.capturedChanges?.length
      ? input.capturedChanges.map((c) => `- ${c}`).join("\n")
      : "(nothing new)"
  }\n\nBR context (JSON):\n${JSON.stringify(input.context, null, 2)}`;
 ...
 * instruction and the full BR context, it returns a conversational reply plus
...
export async function translateSlideContentToLocale(
  ...
  const system = `You translate BR slide content JSON for a client-facing PowerPoint deck into ${target}.
Rules:
...
function buildDeterministicVpSummary(input: {
  clientName: string;
  quarter: string;
  data: any;
}): string {
