 ...
 * (intros, next-action instructions, the "no matching QBR" reply). The agent's
...
const STRINGS: Record<Locale, EmailWorkflowStrings> = {
  en: {
    startedQbr: (client, quarter, year) => `Started the ${client} ${quarter} ${year} QBR.`,
    ...
    draftReplyActions: "Reply APPROVE, REVISE, or FINALIZE — or open the editor to make changes.",
    currentDeckAttached: (fileName) => `Here's the current deck, attached: ${fileName}.`,
    rebuildDeckActions: [
    ...
    noCycleAnswer:
      "I couldn't match your email to an existing QBR. Tell me the client and quarter (e.g. \"McGill Q1 2026\"), or start one with subject: Start QBR - <Client> - Q# YYYY.",
    noCycleNext: "Reply with the client name and quarter so I can pull up the right QBR.",
  },
  fr: {
    startedQbr: (client, quarter, year) => `Le BTR ${client} ${quarter} ${year} a été démarré.`,
    draftAttached: (fileName) =>
    ...
    currentDeckAttached: (fileName) => `Voici la présentation actuelle, en pièce jointe : ${fileName}.`,
    rebuildDeckActions: [
    ...
    finalDeckAttached: (fileName) => `Présentation finale jointe : ${fileName}.`,
    finalizeNext: [
    ...
    surveyQueued: "Je vais mettre en file les sondages de satisfaction client et interne.",
    ...
    noCycleNext: "Répondez avec le nom du client et le trimestre pour que je trouve le bon BTR.",
  },
};

export function getEmailStrings(locale: Locale | undefined): EmailWorkflowStrings {
