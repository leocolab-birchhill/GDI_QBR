...
/**
 * Append a clean hyperlink to the live deck editor on every outbound QBR email.
...
export async function enrichWithEditorLink(args: {
  ...
  locale?: string | null;
}): Promise<{ text: string; html?: string; attachments?: OutboundAttachment[] }> {
  ...
  const newQbrUrl = `${env.APP_URL.replace(/\/$/, "")}/qbr/new`;
  const textLine =
    locale === "fr"
  ...
  const createLine =
    locale === "fr"
      ? `Créer un nouveau client/QBR : ${newQbrUrl}`
      : `Create a new client/QBR: ${newQbrUrl}`;
