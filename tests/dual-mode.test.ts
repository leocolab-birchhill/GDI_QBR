...
describe("dual-mode intent routing", () => {
  it("routes an inbound question to Agent answer mode", () => {
    ...
    const body = "What else do you need from me for the McGill deck this quarter?";
  ...
  it("routes GENERAL_QUESTION to agent regardless of phrasing", () => {
    expect(determineMode("GENERAL_QUESTION", { subject: "x", body: "where are we" })).toBe("agent");
  });
  ...
  it("keeps explicit action requests in workflow mode even when phrased as a question", () => {
    expect(determineMode("REQUEST_DRAFT", { subject: "deck", body: "Generate draft?" })).toBe("workflow");
    expect(determineMode("FINALIZE_DRAFT", { subject: "deck", body: "finalize" })).toBe("workflow");
  });
...
describe("duplicate-prevention helpers", () => {
  ...
  it("treats same label with different case/punctuation as the same item", () => {
    expect(sameItem("Average inspection score", "average inspection score!")).toBe(true);
...
function ctx(overrides: Partial<AnswerContext> = {}): AnswerContext {
  return {
    ...
    upcomingItems: [],
    missingInfo: [
      { field: "priorityItems", question: "2-3 priority items", status: "Open" },
...
describe("agent answer mode", () => {
  it("'what else do you need?' returns a missing-info breakdown, not captured metrics", () => {
    const { answer } = deterministicAnswer("What else do you need from me for the McGill deck this quarter?", ctx());
  ...
  it("'who owns the outstanding items?' lists owners and flags unassigned", () => {
    const { answer } = deterministicAnswer("Who owns the outstanding items?", ctx());
...
describe("buildEmailResponse context header", () => {
  it("every email includes QBR, Status, and Mode lines", () => {
    const { text } = buildEmailResponse({
      qbrContext: { clientName: "McGill University", quarter: "Q1", year: 2026, status: "COLLECTING_INPUTS" },
      mode: "Answer",
      answerText: "Here's the latest.",
    });
    expect(text).toContain("QBR: McGill University — Q1 2026");
  ...
  it("still produces a header when no cycle matched", () => {
    ...
    expect(header).toMatch(/QBR: \(could not match/);
    expect(header).toContain("Mode: Missing info");
  });

  it("does not repeat the same captured line twice", () => {
    const { text } = buildEmailResponse({
      qbrContext: { clientName: "X", quarter: "Q1", year: 2026, status: "COLLECTING_INPUTS" },
      mode: "Captured update",
      capturedItems: ["Health & Safety: Injuries reported = 0", "Health & Safety: Injuries reported = 0"],
    });
  ...
  it("replySubjectFor adds a single Re: prefix", () => {
    ...
    expect(replySubjectFor("Re: McGill Q1 deck", null)).toBe("Re: McGill Q1 deck");
  });
