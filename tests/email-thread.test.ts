...
vi.mock("@/lib/email/providers/graphAuth", () => ({
  ...
  getConnectedAccount: async () => ({ email: "bot@outlook.com", refreshToken: "x" }),
}));
...
describe("Microsoft Graph reply preserves the conversation thread", () => {
  ...
  it("uses createReply on the original message id and threads via conversationId", async () => {
    const { MicrosoftGraphEmailProvider } = await import(
      "@/lib/email/providers/MicrosoftGraphEmailProvider"
    );
    ...
    fetchMock
      .mockResolvedValueOnce(jsonRes({ id: draftId, conversationId: "conv-abc" })) // createReply
    ...
    expect(firstUrl).toContain(`/messages/${encodeURIComponent("original/msg=id")}/createReply`);
  ...
  it("falls back to a fresh send when no original message id is available", async () => {
    ...
    await provider.sendEmail({ to: "user@x.com", subject: "Re: McGill Q1", text: "hi" });
