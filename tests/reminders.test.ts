...
describe("reminder & email generation", () => {
  ...
  it("create confirmation lists the missing items", () => {
    ...
    expect(c.text).toContain("Created: McGill University Q1 2026 BR");
    expect(c.text).toContain("Previous follow-up statuses");
  });

  it("draft-ready email asks for APPROVE/REVISE/FINALIZE", () => {
    const c = tpl.draftReady({
      fileName: "McGill_Q1_2026_BR_Draft_v1.pptx",
      unconfirmed: ["Outstanding invoices"],
    });
    expect(c.text).toContain("McGill_Q1_2026_BR_Draft_v1.pptx");
  ...
  it("VP summary email carries the summary text", () => {
    const c = tpl.vpSummary({
      clientName: "McGill University",
      quarter: "Q1",
      summary: "Status: COLLECTING_INPUTS",
    });
